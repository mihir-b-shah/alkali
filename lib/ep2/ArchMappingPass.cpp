...
class MappingSolver {
 ...
 // Funcs
  ThroughputT getTargetThroughput(FuncOp func) {
    // TODO(): based on requriements, add more constraints
  ...
  void calculateCommunicationOverhead() {
    ...
    for (auto &[func, reps] : mapping) {
      ...
      // TODO(): add support for partition mode
      for (auto &rep : reps) {
  ...
  void calculateCost() {
    ...
    for (auto &[_func, vec] : mapping) {
      // TODO(): different unit and get min?
      auto func = _func;
  ...
  auto extractSolution() {
    ...
    // TODO(): add an assumption of all mapped to same
    z3::check_result res;
    if (all_eq) {
...
void ArchMappingPass::runOnOperation() {
  ...
  // TODO(): add based on search partition policy: partition or not
