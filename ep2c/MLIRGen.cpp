...
private:
  ...
  void update(ExprAST &targetAst, mlir::Value value) {
      ...
      // TODO(): now we do not really have Expr_Var type?
  ...
  mlir::Value mlirGen(BinaryExprAST &binop) {
    ...
    case '=':
      // TODO(): Here we do not bring type info into IR, need to do type
      // check here
      ...
      // TODO(): check this. Is there any other lvalue type?
  ...
  mlir::Value getPath(PathExprAST &path, bool limit = false) {
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (!value) { // only the first call could be value
        ...
        // else, c++ dot access
        // TODO(): multi-level struct access
  ...
  mlir::LogicalResult mlirGen(ExprASTList &blockAST, bool newScope = true) {
    ...
    for (auto &expr : blockAST) {
      ...
      if (auto *vardecl = dyn_cast<VarDeclExprAST>(expr.get())) {
        // TODO(): what's the implication on this?
        // if (!newScope) {
        //   emitError(loc(vardecl->loc()))
